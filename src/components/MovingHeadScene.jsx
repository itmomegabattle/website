...
export default function MovingHeadScene({ onReady }) {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      renderer.setPixelRatio(Math.min(window.devicePixelRatio || 1, isCompactViewport ? 1.5 : 2));
      ...
      model.scale.setScalar((isCompactViewport ? 0.55 : 1.18) / maxDimension);
      model.position.y = isCompactViewport ? -0.39 : -0.52;
      ...
      floorShadow.position.set(0, isCompactViewport ? -0.61 : -0.94, 0.03);
      ...
      const render = (now) => {
        ...
        model.position.y = (isCompactViewport ? -0.39 : -0.52) + idleFloat;
