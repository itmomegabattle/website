...
export default function InlineEventEditor({ groupId, selectedEvent, onClose }) {
  ...
  const [detailsText, setDetailsText] = useState(() => splitEventDetails(selectedEvent?.details).details.join("\n"));
  ...
  const editor = (
    <div className="event-editor-overlay" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose?.()}>
    <form className="event-inline-editor" onSubmit={handleSubmit} role="dialog" aria-modal="true" aria-label={selectedEvent ? "Редактировать мероприятие" : "Новое мероприятие"}>
      <div className="event-inline-editor-head">
        <div><p className="card-kicker">Управление событием</p><h2>{selectedEvent ? "Редактировать мероприятие" : "Новое мероприятие"}</h2></div>
      ...
      <div className="event-inline-editor-grid">
        ...
        <label className="form-field"><span>Текст кнопки Telegram</span><input name="telegram_label" value={form.telegram_label || ""} onChange={updateField} /></label>
        <label className="form-field"><span>Ссылка Telegram</span><input name="telegram_link" value={form.telegram_link || ""} onChange={updateField} placeholder="https://t.me/..." /></label>
